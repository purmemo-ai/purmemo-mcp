...
// ============================================================================
// Shadow mirror (purmemo rebuild — "the Door")
// ============================================================================
//
// After a REAL cloud save succeeds, fire a COPY of the content at the rebuilt
// engine's front door. This is strictly additive telemetry: it is never awaited
// into the user's tool response, every failure is swallowed to a local log, and
// with the two env vars absent it is a total no-op (zero behavior change). tsup
// + CJS deps have burned prod before, so this uses ONLY Node built-in fetch
// (Node >=18) and adds NO dependency.

const SHADOW_LOG = `${process.env.HOME || ''}/.claude/state/purmemo-shadow.log`;

function shadowLog(line: string) {
  try {
    appendFileSync(SHADOW_LOG, `${new Date().toISOString()} ${line}\n`);
  } catch {
    // A logging failure must never propagate. Nothing else to do.
  }
}
...
function fireShadowDoor(content: string, saveResult?: any): void {
  try {
    ...
    // Not awaited. The .then/.catch only touch the local log.
    fetch(url, {
      method: 'POST',
      headers: {
      ...
      .then((resp) => {
        clearTimeout(timer);
        if (!resp || !resp.ok) {
          shadowLog(`shadow non-ok status=${resp ? resp.status : 'none'}`);
        }
      })
      .catch((err) => {
        clearTimeout(timer);
        shadowLog(`shadow error ${err && err.name === 'AbortError' ? 'timeout' : (err && err.message) || String(err)}`);
      });
  } catch (err: any) {
    // Synchronous failure (env read, JSON.stringify, fetch construction) —
    // log and swallow. NEVER throws, NEVER alters the tool result.
    shadowLog(`shadow sync-fail ${(err && err.message) || String(err)}`);
  }
}
...
async function saveChunkedContent(content, title, tags = [], metadata = {}) {
  ...
  // ADR-036: forward mode to every part. handleSaveConversation rejects
  // mode='append' before reaching this path (append doesn't compose with
  // chunked storage — would double content on re-save). This forwarding
  // remains defensive in case saveChunkedContent is called from another
  // path in the future, and so the mode value flows correctly to the API.
  const mode = metadata._mode || 'replace';
...
export async function handleSaveConversation(args) {
  ...
  try {
    ...
    // raw Server class does not auto-validate inputSchema enum, so reject
    // invalid mode values loudly here rather than silently falling through.
    if (args.mode !== undefined) {
      if (args.mode !== 'append' && args.mode !== 'replace') {
        return {
          content: [{
            type: 'text',
            text: `❌ Invalid mode: "${args.mode}". Must be "replace" or "append".`
          }]
        };
      }
      metadata._mode = args.mode;
    }
    ...
    // ADR-036: append mode does not compose with chunked storage. Chunked saves
    // re-derive `${slug}:part:N` from the title slug, so a re-save of the same
    // content with mode='append' would append the entire content onto existing
    // part rows, doubling each part. The right semantics here is to require the
    // caller to either send a smaller delta (<15K, single-path append works) or
    // use mode='replace' for full re-save. This is a known limitation tracked
    // alongside ADR-038 (uniform namespace would resolve it).
    if (shouldChunk(content) && metadata._mode === 'append') {
      return {
        content: [{
          type: 'text',
          text: `❌ APPEND MODE NOT SUPPORTED FOR CHUNKED SAVES (>15K chars)\n\n` +
                `Content length: ${contentLength} chars (chunking threshold: 15,000).\n\n` +
                `Append works at the row level — but content >15K splits across multiple\n` +
                `rows whose conversation_ids are derived from the title. Re-saving with\n` +
                `mode='append' would double each row's content.\n\n` +
                `OPTIONS:\n` +
                `  1. Send only the new delta since last save (must be <15K chars).\n` +
                `     This is the recommended path for /save workflows on long-running docs.\n` +
                `  2. Use mode='replace' for a full re-save of the entire conversation.\n` +
                `     Prior content is preserved in memory_events audit log.\n\n` +
                `Tracked: ADR-038 (uniform conversation_id namespace) will resolve this.`
        }]
      };
    }
    ...
    if (shouldChunk(content)) {
      ...
      return {
        content: [{
          ...
          text: `✅ LARGE CONVERSATION SAVED (Auto-chunked)!\n\n` +
                ...
                (conversationId ? `🧭 Mode: ${metadata._mode || 'replace'} — next save with same title will ${metadata._mode === 'append' ? 'APPEND below this content' : 'REPLACE this content'}\n` : '') +
      ...
      const effectiveMode = metadata._mode || 'replace';
      const updateVerb = effectiveMode === 'append' ? 'APPENDED to' : 'REPLACED';
      const savedOrUpdated = result.wasUpdated
        ? `✅ CONVERSATION ${updateVerb} EXISTING MEMORY!\n\n` +
          (conversationId ? `📝 Conversation ID: ${conversationId}` + (isAutoGenerated ? ' (auto-generated from title)\n' : '\n') : '') +
          `🧭 Mode: ${effectiveMode}\n` +
          `📏 ${effectiveMode === 'append' ? 'New content size' : 'New size'}: ${result.size} characters\n` +
          ...
          (effectiveMode === 'append'
            ? `✓ Appended below prior content with timestamped separator. Prior content preserved inline.`
            : `✓ Existing content overwritten. Prior content snapshotted to memory_events audit log.`)
        : `✅ CONVERSATION SAVED!\n\n` +
          (conversationId ? `📝 Conversation ID: ${conversationId}` + (isAutoGenerated ? ' (auto-generated from title)\n' : '\n') : '') +
          `🧭 Mode: ${effectiveMode} — next save with same title will ${effectiveMode === 'append' ? 'APPEND below this content' : 'REPLACE this content'}\n` +
